...
class WMCompressionSolver(base.StandardSolver):
    """Solver for watermark compression task.
    ...
    def __init__(self, cfg: omegaconf.DictConfig):
        ...
        self.cls_losses = nn.CrossEntropyLoss()
    ...
    def build_model(self, checkpoint_path='/apdcephfs_cq10/share_1603164/user/helinhwang/VoiceCraft/pretrained_models/VoiceCraft/encodec_4cb2048_giga.th'):
        ...
        if checkpoint_path is not None:
            self.model = self.model_from_encodec_checkpoint(checkpoint_path=checkpoint_path, device=self.device)
        else:
            self.model = models.builders.get_wmcompression_model(self.cfg).to(self.device)
    ...
    def show(self):
        ...
        self.logger.info("Classifier losses:")
        self.logger.info(self.cls_losses)
    ...
    def model_from_checkpoint(checkpoint_path: tp.Union[Path, str],
        ...
        compression_model = models.builders.get_wmcompression_model(cfg).to(device)
    ...
    @staticmethod
    def model_from_encodec_checkpoint(checkpoint_path: tp.Union[Path, str],
                              device: tp.Union[torch.device, str] = 'cpu') -> models.CompressionModel:
        """Instantiate a CompressionModel from a given checkpoint path or dora sig.
        This method is a convenient endpoint to load a CompressionModel to use in other solvers.
        ...
        for name, param in encodec_state_dict.items():
            if name in wmencodec_state_dict:
                wmencodec_state_dict[name].copy_(param)
    ...
    def wrapped_model_from_checkpoint(cfg: omegaconf.DictConfig,
        ...
        Args:
            ...
            device (torch.device or str): Device on which the model is loaded.
        """
        compression_model = WMCompressionSolver.model_from_checkpoint(checkpoint_path, device)
        compression_model = models.builders.get_wrapped_wmcompression_model(compression_model, cfg)
        return compression_model
