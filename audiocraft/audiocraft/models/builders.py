...
from .wmencodec import WMCompressionModel, WMEncodecModel
...
def get_wmencodec_autoencoder(encoder_name: str, cfg: omegaconf.DictConfig):
    if encoder_name == 'seanet':
        kwargs = dict_from_config(getattr(cfg, 'seanet'))
        encoder_override_kwargs = kwargs.pop('encoder')
        decoder_override_kwargs = kwargs.pop('decoder')
        ...
        wmdecoder = audiocraft.audiocraft.modules.WMSEANetDecoder(**decoder_kwargs)
        return encoder, decoder, wmdecoder
    else:
        raise KeyError(f"Unexpected compression model {cfg.compression_model}")
...
def get_wmcompression_model(cfg: omegaconf.DictConfig) -> WMCompressionModel:
    """Instantiate a compression model."""
    if cfg.compression_model == 'wmencodec':
        kwargs = dict_from_config(getattr(cfg, 'encodec'))
        encoder_name = kwargs.pop('autoencoder')
        quantizer_name = kwargs.pop('quantizer')
        encoder, decoder, wmdecoder = get_wmencodec_autoencoder(encoder_name, cfg)
        quantizer = get_quantizer(quantizer_name, cfg, encoder.dimension)
        frame_rate = kwargs['sample_rate'] // encoder.hop_length
        renormalize = kwargs.pop('renormalize', False)
        # deprecated params
        kwargs.pop('renorm', None)
        return WMEncodecModel(encoder, decoder, wmdecoder, quantizer,
                            frame_rate=frame_rate, renormalize=renormalize, **kwargs).to(cfg.device)
    else:
        raise KeyError(f"Unexpected compression model {cfg.compression_model}")
...
def get_wrapped_wmcompression_model(
        wmcompression_model: WMCompressionModel,
        cfg: omegaconf.DictConfig) ->WMCompressionModel:
    # more to come.
    return compression_model
